    ...
    jQuery(function(e) {
        if (e(".audio-player").length) {
            ...
            e(".add_to_queue").on("click", function() {
                ...
                a += "&action=miraculous_add_to_queue_action", e(".ms_ajax_loader").show(), e.
				ajax({
                    type: "post",
                    url: frontadminajax.ajax_url,
                    data: a,
                ...
                var playbackState = localStorage.getItem("jPlayerPlaybackState");
				var playbackState = JSON.parse(playbackState);
				if (playbackState.paused == "true") {
                ...
                t("MJTCURRENTSONG", r.current, 1), a("#jquery_jplayer_1").bind(a.jPlayer.event.timeupdate, function(a) {
                    if(duration != ''){
                     ...
                     if(a.jPlayer.status.currentTime>0 && a.jPlayer.status.paused===false) {
                         var myVar = "true";
                     }
                    var playbackState = {
                        currentTime: a.jPlayer.status.currentTime,
                        paused: false
                    };
                    localStorage.setItem("jPlayerPlaybackState", JSON.stringify(playbackState));
