...
class Miraculous_setting {
  /**
	...
	public function miraculous_header_setting(){
		...
		if ( class_exists( 'ReduxFramework' ) ) {
			$miraculous_data = '';
			$miraculous_data = get_option('miraculous_options');
		}
		...
		switch ($header_style):
			...
			case 'style-one':
				...
				break;
			case 'style-two':
				get_template_part( 'vendor/header/miraculous-header', 'style-two');
				break;	
			default:
				get_template_part( 'vendor/header/miraculous-default', 'header' );
		endswitch;
	...
	public function miraculous_breadcrumbs_setting(){
		...
		if(class_exists( 'ReduxFramework' )){
			...
			if(!empty($miraculous_theme_data['breadcrumbs_switch'])):
			   if($miraculous_theme_data['breadcrumbs_switch'] == 1):
				  $breadcrumbs =  'on';
			   else:
			      $breadcrumbs =  'off';
			   endif;
			...
			if(!empty($miraculous_pagetitle_override)){
				if( is_page() && ($miraculous_pagetitle_override[0] == 1) || is_single() && ($miraculous_pagetitle_override[0] == 1) ):
					if(!empty($pagetitle_meta_switch)):
						if(is_page() && ($pagetitle_meta_switch[0] == 0)):
						   $breadcrumbs =  'off';
						else:
						  $breadcrumbs =  'on'; 
						endif;
					endif;
					if(!empty($post_breadcrumbs_switch)):
						if(is_single() && ($post_breadcrumbs_switch[0] == 0)):
						   $breadcrumbs =  'off';
						else:
						  $breadcrumbs =  'on'; 
						endif;
	...
	public function miraculous_index_blog(){
		...
		extract($data);
		if(class_exists( 'ReduxFramework' )){
			if(!empty($miraculous_page_sidebar_override)){
				if($miraculous_page_sidebar_override[0] == 1){
					if(!empty($page_meta_sidebar_postion)){
						if($page_meta_sidebar_postion[0] == 'left'){
							$theme_sidebar = esc_html__('left','miraculous');
						}
						if($page_meta_sidebar_postion[0] == 'right'){
							$theme_sidebar = esc_html__('right','miraculous');
						}
					}else{
						$theme_sidebar = esc_html__('full','miraculous');
					}
				}
			}
		}
	...
	public function miraculous_blog_single_setting($blog_id){
		...
		echo '<div class="ms_main_data">';
		        while ( have_posts() ) : the_post();
					...
					if(get_post_type()== 'ms-music' || get_post_type()== 'ms-artists' || get_post_type()== 'ms-albums' || get_post_type()== 'ms-podcasts'):
    ...
    public function miraculous_footer_setting(){
		...
		if(class_exists('Miraculouscore')):
		   $miraculous_core = new Miraculouscore();
			...
			$miraculous_core->miraculous_Single_buy_popup();
		endif;
    }  
}