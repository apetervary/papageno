	...
	class ReduxFramework_extension_metaboxes {
		...
		function url_to_postid( $url ) {
			...
			foreach ( (array) $rewrite as $match => $query ) {
				...
				if ( isset($matches) && preg_match( "!^$match!", $request_match, $matches ) ) {
		...
		function get_values( $thePost, $meta_key = "" ) {
			// Override these values if they differ from the admin panel defaults.  ;)
			if ( !empty($thePost) && in_array( $thePost->post_type, $this->post_types ) ) {
							...
							<ul class="redux-group-menu">
								<?php
									foreach ( $sections as $sKey => $section ) {
										echo $this->parent->render_class->section_menu( $sKey, $section, '_box_' . $metabox['id'], $sections );
									}
								?>
							</ul>
						</div>
...
add_action('redux/metaboxes/miraculous_options/boxes',  'miraculous_redux_add_metaboxes' );

function miraculous_redux_add_metaboxes($metaboxes) {
	...
	$boxSections     = array();
	$musicSections   = array();
	$artistSections  = array();
	$albumSections   = array();
	$radioSections   = array();
	$podcastSections = array();
	$planSections    = array();
	...
	if($post_type == 'page'){
		...
		$boxSections[] = array(
			'title' => __( 'Revolution Slider', 'miraculous' ),
			'id'    => 'miraculous_revslider_settings',
			'icon'  => 'el el-credit-card',
			'fields'           => array(
				array(
					'id'       => 'miraculous_revslider',
					'type'     => 'select',
					'title'    => __( 'Revolution Slider', 'miraculous' ),
					'options'     => $rev_slider,
				),
		...
		$boxSections[] = array(
			'title' => __( 'Title/Breadcrumb', 'miraculous' ),
			'id'    => 'miraculous_pagetitle_settings',
			'icon'  => 'el el-credit-card',
			'fields'           => array(
				array(
					'id'       => 'miraculous_pagetitle_override',
					'type'     => 'switch',
					'title'    => 'Override Title/Breadcrumb',
					'default'  => false
				),
				array(
				   'id' => 'pagetitle_meta_main_section',
				   'type' => 'section',
				   'title' => __('Page Title', 'miraculous'),
				   'indent' => true,
				   'required' => array( 'miraculous_pagetitle_override', '=', true )
				),
				array(
					'id'       => 'pagetitle_meta_switch',
					'type'     => 'switch',
					'on' 	   => 'On',
					'off' 	   => 'Off',
					'title'    => 'Page Title',
		...
		$boxSections[] = array(
			...
			'id'    => 'miraculous_general_sidebar',
			'icon'          => 'dashicons dashicons-align-left',
			'fields'     => array(
				array(
					'id'       => 'miraculous_page_sidebar_override',
					'type'     => 'switch',
					'title'    => 'Override Page Sidebar',
					'default'  => false
				),
				array(
					'id'       => 'page_meta_sidebar_postion',
					'type'     => 'image_select',
					'title'    => __( 'Page Sidebar Position', 'miraculous' ),
					'options'  => array(
						'full' => array(
							'alt' => 'No sidebar',
					...
					'default'  => 'full',
					'required' => array( 'miraculous_page_sidebar_override', '=', true )
				),
			)
		);
		...
		$boxSections[] = array(
			...
			'fields'           => array(
				...
				array(
					'id'       => 'post_bgimages_switch',
					'type'     => 'switch',
					'title'    => __( 'Background Image Enable/Disable', 'miraculous' ),
					'default'  => true,
				...
				array(
					...
					'type'     => 'media',
					'url'      => true,
					'title'    => __( 'Page Title Image', 'miraculous'),
				),
			)
		);
		$boxSections[] = array(
	...
	$musicSections = array();
	//Music type meta field
	if($post_type == 'ms-music'){
		$artist = '';
		if(function_exists('miraculous_artist_list')):
			$artist = miraculous_artist_list();
		endif;
		$musicSections[] = array(
			'title' => __( 'Music Upload Options', 'miraculous' ),
			'id'    => 'music_upload_option',
			'icon'  => 'el el-credit-card',
			'fields'           => array(
				array(
					'id'       => 'mp3_full_songs',
					'type'     => 'media', 
					'title'    => __('Mp3 Full song', 'miraculous'),
					'library_filter' => array('mp3', 'wav', 'ogg', 'm4a'),
					'default'  => false
				),
				array(
					'id'       => 'music_extranal_url',
					'type'     => 'text', 
					'url'      => true,
					'title'    => __('Mp3 Music External Url', 'miraculous'),
					'default'  => false
				),
				array(
					'id'       => 'user_music_artist',
					'type'     => 'text', 
					'url'      => true,
					'title'    => __('Artist Name', 'miraculous'),
					'default'  => false
				),
			)
		);
		$musicSections[] = array(
			...
			'fields'     => array(
				...
				array(
					'id'       => 'single_music_prices',
					'type'     => 'text', 
					'url'      => true,
					'title'    => __('Price', 'miraculous'),
					'desc'     => __('Enter Music Price.', 'miraculous')
				),
				array(
					'id'       => 'music_artists',
					'type'     => 'select',
				...
				array(
					...
					'type'        => 'date',
					'title'       => __('Release Date', 'miraculous'), 
	...
	//Artist type meta field
	if($post_type == 'ms-artists'){
		
		$artistSections[] = array(
			'title' => __( 'Artist Options', 'miraculous' ),
			'id'    => 'miraculous_artistoption',
			'icon'  => 'el el-credit-card',
			'fields'           => array(
				array(
					'id'          => 'artist_born_date',
					'type'        => 'date',
					'title'       => __('Born Date', 'miraculous'), 
					'placeholder' => 'Click to enter a date'
				),
				array(
					'id'          => 'artist_born_place',
					'type'        => 'text',
					'title'       => __('Place of Born', 'miraculous'), 
				),
			)
		);
		$artistSections[] = array(
			'title' => __( 'Theme Options', 'miraculous' ),
			'id'    => 'miraculous_artistheme_option',
			'icon'          => 'dashicons dashicons-align-left',
			'fields'     => array(
				array(
					'id'       => 'page_meta_sidebar_postion',
	...
	if($post_type == 'ms-albums'){
		...
		$music = '';
		if(function_exists('miraculous_music_list')):
			$music = miraculous_music_list();
		endif;
		$albumSections[] = array(
			'title' => __( 'Album Options', 'miraculous' ),
			'id'    => 'miraculous_albumoption',
			'icon'  => 'el el-credit-card',
			'fields'           => array(
				array(
				...
				array(
					'id'          => 'album_company_name',
					'type'        => 'text',
					'title'       => __('Company Name', 'miraculous'), 
				),
				array(
					'id'       => 'album_type',
					'type'     => 'radio',
					...
					'default' => __('premium', 'miraculous'),
				),
				array(
					'id'          => 'album_full_prices',
					'type'        => 'text',
					'title'       => __('Album Price', 'miraculous'), 
				),
				array(
					'id'       => 'album_songs',
					'type'     => 'select',
					'multi'    => true,
					'title'    => __('Song Name', 'miraculous'), 
		...
		$albumSections[] = array(
			'title' => __( 'Theme Options', 'miraculous' ),
			'id'    => 'miraculous_albumtheme_option',
			'icon'          => 'dashicons dashicons-align-left',
			'fields'     => array(
				array(
					'id'       => 'page_meta_sidebar_postion',
	...
	if($post_type == 'ms-radios'){
		...
		$radioSections[] = array(
			...
			'fields'           => array(
				array(
					'id'          => 'radio_release_date',
					'type'        => 'date',
					'title'       => __('Release Date', 'miraculous'), 
					'placeholder' => 'Click to enter a date'
				),
				array(
					'id'          => 'radio_company_name',
					'type'        => 'text',
					'title'       => __('Company Name', 'miraculous'), 
				),
				array(
	...
	//Podcast type meta field
	if($post_type == 'ms-podcasts'){
		$artist = '';
		if(function_exists('miraculous_artist_list')):
			$artist = miraculous_artist_list();
		endif;
		...
		$podcastSections[] = array(
			...
			'fields'           => array(
				array(
					'id'          => 'podcast_release_date',
					'type'        => 'date',
					'title'       => __('Release Date', 'miraculous'), 
					'placeholder' => 'Click to enter a date'
				),
				array(
					'id'          => 'podcast_company_name',
					'type'        => 'text',
					'title'       => __('Company Name', 'miraculous'), 
				),
				array(
	...
	//Plan type meta field
	if($post_type == 'ms-plans'){
		$planSections[] = array(
			...
			'fields'           => array(
				array(
					'id'       => 'free_plane_switch',
					'type'     => 'switch',
					'title'    => __( 'Free Plan Enable/Disable', 'miraculous' ),
					'default'  => true,
				),
				array(
					'id'          => 'plan_monthly_price',
					'type'        => 'text',
					'title'       => __('Monthly Price', 'miraculous'),
					'desc'        => __('Enter monthly price eg. 20, Do not use decimal value.', 'miraculous'),
				),
				array(
					'id'          => 'plan_monthly_uploads',
					'type'        => 'text',
					'title'       => __('Monthly Uploads', 'miraculous'), 
					'desc'        => __('Enter number of songs to Uploads eg. 50', 'miraculous'),
				),
				array(
					'id'          => 'plan_monthly_download',
					'type'        => 'text',
					'title'       => __('Monthly Download', 'miraculous'), 
					'desc'        => __('Enter number of songs to download Limite eg. 50', 'miraculous'),
				),
				array(
					'id'          => 'plan_validity',
					'type'        => 'text',
					'title'       => __('Plan Validity', 'miraculous'), 
		...
		$productSections[] = array(
			...
			'fields'           => array(
				array(
					'id'       => 'woo_audio',
					'type'     => 'switch',
					'title'    => __( 'WooCommerce Page MP3 Enable/Disable', 'miraculous' ),
					'default'  => true,
				),
				array(
					'id'       => 'mp3_full_songs',
	...
	$metaboxes[] = array(
		'id'            => 'ms_artists_fields',
		'title'         => __( 'Artist Options', 'miraculous' ),
		'post_types'    => array( 'ms-artists' ),
		'position'      => 'normal', // normal, advanced, side
		'priority'      => 'high', // high, core, default, low - Priorities of placement
	...
	$metaboxes[] = array(
		'id'            => 'ms_radio_fields',
		'title'         => __( 'Radio Options', 'miraculous' ),
		'post_types'    => array( 'ms-radios' ),
		'position'      => 'normal', // normal, advanced, side
		'priority'      => 'high', // high, core, default, low - Priorities of placement
		'sections'      => $radioSections,
	);
	$metaboxes[] = array(
		'id'            => 'ms_podcast_fields',
		'title'         => __( 'Podcast Options', 'miraculous' ),
		'post_types'    => array( 'ms-podcasts' ),
		'position'      => 'normal', // normal, advanced, side
		'priority'      => 'high', // high, core, default, low - Priorities of placement
	...
	$metaboxes[] = array(
		'id'            => 'ms_plan_fields',
		'title'         => __( 'Plan Options', 'miraculous' ),
		'post_types'    => array( 'ms-plans' ),
		'position'      => 'normal', // normal, advanced, side
		'priority'      => 'high', // high, core, default, low - Priorities of placement
		'sections'      => $planSections,
	);
	$metaboxes[] = array(
		'id'            => 'ms_product_fields',
		'title'         => __( 'Music Upload Options', 'miraculous' ),
